    ...
    @foreach($contrato->historicos as $h)
        @foreach($h->pecasAvulsas as $s)
            <tr class="align-middle " >
                ...
                <td style="{{isset($peca_avulsa_alterada_id)?$peca_avulsa_alterada_id==$s->id?'background-color: #c5ffcd':'':''}}"><input REQUIRED ativo="valor-bruto" peca_id="{{$s->id}}" class="form-control form-control-sm calcular-valors-peca" id="peca-valor-bruto-{{$s->id}}" name="valor-bruto-{{$s->id}}" value="{{$s->valor_bruto}}"></td>

                <td style="{{isset($peca_avulsa_alterada_id)?$peca_avulsa_alterada_id==$s->id?'background-color: #c5ffcd':'':''}}"><input REQUIRED ativo="desconto" peca_id="{{$s->id}}" class="form-control form-control-sm calcular-valors-peca" id="peca-desconto-{{$s->id}}" name="desconto-{{$s->id}}" value="{{$s->desconto}}"></td>
                <td style="{{isset($peca_avulsa_alterada_id)?$peca_avulsa_alterada_id==$s->id?'background-color: #c5ffcd':'':''}}"><input REQUIRED ativo="valor-liquido" peca_id="{{$s->id}}" class="form-control form-control-sm calcular-valors-peca" id="peca-valor-liquido-{{$s->id}}" name="valor-liquido-{{$s->id}}" value="{{$s->valor_liquido}}"></td>
                <td style="{{isset($peca_avulsa_alterada_id)?$peca_avulsa_alterada_id==$s->id?'background-color: #c5ffcd':'':''}}"><input REQUIRED ativo="qnt" peca_id="{{$s->id}}" class="form-control form-control-sm calcular-valors-peca" id="peca-qnt-{{$s->id}}" name="qnt-{{$s->id}}" value="{{$s->qnt}}"></td>
                <td style="{{isset($peca_avulsa_alterada_id)?$peca_avulsa_alterada_id==$s->id?'background-color: #c5ffcd':'':''}}">{{$s->valor_liquido*$s->qnt}}</td>
